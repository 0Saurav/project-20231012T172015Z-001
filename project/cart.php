...
if (isset($_POST['update_cart'])) {
   ...
   $cart_query = mysqli_query($conn, "SELECT c.id, c.name, c.price, c.quantity, c.image, p.stock FROM cart c
                                      JOIN products p ON c.name = p.name
                                      WHERE c.id = '$cart_id' AND c.user_id = '$user_id'") or die('query failed');
   $cart_item = mysqli_fetch_assoc($cart_query);

   if ($cart_quantity > $cart_item['stock']) {
